...
func TestUnmarshalPeersMalformed(t *testing.T) {
	cases := []struct {
		...
		{
			// Present peer IDs must be strings.
			name:      "InvalidPeerID",
			input:     "d5:peersld2:ip7:1.2.3.47:peer idi1e4:porti6881eeee",
			wantCount: 0,
		},
		{
			// Ports outside the unsigned 16-bit range are not connectable.
			name:      "PortOutOfRange",
			input:     "d5:peersld2:ip7:1.2.3.44:porti65536eeee",
			wantCount: 0,
		},
	}
